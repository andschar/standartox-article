...
%=================================================================
\documentclass[journal,datadescriptor,submit,moreauthors,pdftex]{Definitions/mdpi} 
...
\Author{Andreas Scharmüller $^{1,2}$*\orcidA{}, Verena C. Schreiner$^{1}$\orcidB and Ralf B. Schäfer $^{1}$\orcidC{}}
...
$^{2}$ \quad Laboratoire d’Hydrologie et de Géochimie de Strasbourg, UMR 7517 CNRS-Université de Strasbourg, 1 rue Blessig, 67084 Strasbourg Cedex, France}
...
\abstract{An increasing number of chemicals such as pharmaceuticals, pesticides and synthetic hormones are in daily use all over the world. In the environment, chemicals can adversely affect populations and communities and in turn related ecosystem functions. To evaluate the risks from chemicals for ecosystems, data on their toxicity, which is typically produced in standardized ecotoxicological laboratory tests, is required. The results from ecotoxicological tests are compiled in (Meta-)databases such as the United States Environmental Protection Agency (EPA) ECOTOXicology knowledgebase (ECOTOX). However, for many chemicals, multiple ecotoxicity data are available for the same test organism. These  can vary strongly, thereby causing uncertainty of related analyses. Given that most current databases lack aggregation steps or are confined to specific chemicals, we developed Standartox, a tool and database that continuously incorporates the ever-growing number of test results in an automated process workflow that ultimately leads to a single aggregated data point for a specific chemical-organism test combination, representing the toxicity of a chemical. Standartox comes with two front-ends, the web application \url{standartox.uni-landau.de} and the R package \textit{standartox}.}
...
\dataset{\url{ftp://newftp.epa.gov/ecotox/ecotox_ascii_12_12_2019.exe},
\url{https://doi.org/10.5281/zenodo.3785031}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/standartox_parameters.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figures/gg_ppdb_stan_compare_continous.png}
...
\begin{tabular}{cc}
    ...
    freshwaterecology.info & \url{www.freshwaterecology.info} \\
    \bottomrule
\end{tabular}
\end{table}
